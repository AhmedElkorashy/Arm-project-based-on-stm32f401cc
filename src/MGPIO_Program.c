...
/*******************************************************************/
void MGPIO_voidTogglePinValue(u8 copyu8Port, u8 copyu8Pin)
{
	if((copyu8Port == GPIO_PORTA) && ((copyu8Pin == GPIO_PIN13) ||(copyu8Pin == GPIO_PIN14)||(copyu8Pin == GPIO_PIN15)))
	{
		/*Do Nothing*/
...
/*********************************/
void MGPIO_voidSetPortValue(u8 copyu8Port, u32 copyu16Output)
{
...
/************************************************************/
void MGPIO_voidSetPortMode(u8 copyu8Port,  u8 copyu8Mode)
{
	switch(copyu8Port)
	{
	case GPIO_PORTA:
		GPIOA->MODER &= ~(TWO_BIT_MASK <<(copyu8Mode *2));
		GPIOA->MODER |= (copyu8Mode<<(copyu8Mode *2));
		break;

	case GPIO_PORTB:
		GPIOB->MODER &= ~(TWO_BIT_MASK <<(copyu8Mode *2));
		GPIOB->MODER |= (copyu8Mode<<(copyu8Mode *2));
		break;

	case GPIO_PORTC:
		GPIOC->MODER &= ~(TWO_BIT_MASK <<(copyu8Mode *2));
		GPIOC->MODER |= (copyu8Mode<<(copyu8Mode *2));
		break;
...
void MGPIO_voidSetPinAlternateFunctionMode(u8 copyu8Port, u8 copyu8Pin, u8 copyu8AltFuncMode)
{
	...
	{
		if( (copyu8Pin >= GPIO_PIN0) && (copyu8Pin <= GPIO_PIN7) ){
			switch(copyu8Port)
			{
		...
		else if( (copyu8Pin >= GPIO_PIN8) && (copyu8Pin <= GPIO_PIN15) )
		{
			switch(copyu8Port)
			{
			case GPIO_PORTA:
				GPIOA->AFRH &= ~(FOUR_BIT_MASK <<(copyu8Pin *4));
				GPIOA->AFRH |= (copyu8AltFuncMode<<(copyu8Pin *4));
				break;

			case GPIO_PORTB:
				GPIOB->AFRH &= ~(FOUR_BIT_MASK <<(copyu8Pin *4));
				GPIOB->AFRH |= (copyu8AltFuncMode<<(copyu8Pin *4));
				break;

			case GPIO_PORTC:
				GPIOC->AFRH &= ~(FOUR_BIT_MASK <<(copyu8Pin *4));
				GPIOC->AFRH |= (copyu8AltFuncMode<<(copyu8Pin *4));
				break;
